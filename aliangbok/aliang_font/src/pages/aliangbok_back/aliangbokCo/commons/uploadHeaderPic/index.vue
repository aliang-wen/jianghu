...
<script>
import { uploadHeaderPic } from '../../../../../network/aliangbok_back/aliangbokCo/uploadHeaderPic/index';
    ...
    finish(type) {
      ...
      if (type === 'blob') {
        this.$refs.cropper.getCropBlob((data) => {
          ...
          // 这里走的是上传的方法
          this.uploadHeaderPicFun(data, this.fileCuuid, this.userId);
        });
      } else {
        this.$refs.cropper.getCropData((data) => {
          ...
          // 这里走的是上传的方法
          ((data || '').length > 0) && this.uploadHeaderPicFun(data, this.fileCuuid, this.userId);
        });
      }
    },
    uploadHeaderPicFun(val, fileId, userId) {
      let params = {
      ...
      uploadHeaderPic(params).then(data => {
        this.$emit('successFun', data);
      }).catch(() => {
